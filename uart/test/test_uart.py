...
class UArtEnv(pyuvm.uvm_env):
    def build_phase(self):
        ...
        self.ready_valid_tx_frame_sequencer = pyuvm.uvm_sequencer('ready_valid_tx_frame_sequencer', self)
        self.ready_valid_tx_frame_interface = pvm.interfaces.ReadyValidDataInterface(cocotb.top.clk, cocotb.top.ready_tx_frame, cocotb.top.valid_tx_frame, cocotb.top.tx_frame)
        self.ready_valid_tx_frame_driver = pvm.drivers.Driver('ready_valid_tx_frame_driver', self, self.ready_valid_tx_frame_interface)
        self.ready_valid_tx_frame_monitor = pvm.monitors.Monitor('ready_valid_tx_frame_monitor', self, self.ready_valid_tx_frame_interface)
        ...
        self.model = UArtModel('model', self)
    ...
    def connect_phase(self):
        ...
        self.ready_valid_tx_frame_driver.seq_item_port.connect(self.ready_valid_tx_frame_sequencer.seq_item_export)
        self.ready_valid_tx_frame_monitor.analysis_port.connect(self.scoreboard.expected_data_analysis_fifo.analysis_export)
...
class UArtTest(pyuvm.uvm_test):
    ...
    async def run_phase(self):
        ...
        await self.fast_transmitter_sequence.start(self.env.ready_valid_tx_frame_sequencer)
        self.drop_objection()

def test_uart_tb_runner():
    runner = cocotb_tools.runner.get_runner('ghdl')

    current_dir = pathlib.Path().absolute()

    runner.test(test_module = 'test_uart',
                hdl_toplevel = 'uart_tb',
                hdl_toplevel_library = 'hdl_uart_tb',
