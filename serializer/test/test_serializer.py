...
class SerializerValidDataSequence(pvm.sequences.ValidDataSequence):
    def __init__(self, name):
        super().__init__(name)
        operand_data_width = int(cocotb.top.OPERAND_DATA_WIDTH.value)
        result_width = int(cocotb.top.RESULT_WIDTH.value)
        self._max_operand_number_of_words = int(operand_data_width / result_width) - 1
...
class SerializerModel(pyuvm.uvm_subscriber):
    ...
    def write(self, item):
        ...
        number_of_words, data = int(number_of_words), int(data)
        number_of_words += 1
        for i in range(number_of_words):
            self.analysis_port.write(data & self._result_mask)
            data = data >> self._result_width

class SerializerEnv(pyuvm.uvm_env):
    def build_phase(self):
        ...
        self.model = SerializerModel('model', self)
...
@pyuvm.test()
class SerializerTest(pyuvm.uvm_test):
    def build_phase(self):
        operand_data_width = int(cocotb.top.OPERAND_DATA_WIDTH.value)
        result_width = int(cocotb.top.RESULT_WIDTH.value)
        number_of_ready_transactions_per_valid_data_transaction = int(operand_data_width / result_width)
        max_number_of_valid_data_transactions = 16
        max_number_of_ready_transactions = number_of_ready_transactions_per_valid_data_transaction * max_number_of_valid_data_transactions
        ...
        pyuvm.uvm_factory().set_type_override_by_type(pvm.sequences.ValidDataSequence, SerializerValidDataSequence)
        self.env = SerializerEnv('env', self)
...
def test_serializer_tb_runner():
    runner = cocotb_tools.runner.get_runner('ghdl')

    current_dir = pathlib.Path().absolute()

    runner.test(test_module = 'test_serializer',
                hdl_toplevel = 'serializer_tb',
                hdl_toplevel_library = 'hdl_serializer_tb',
