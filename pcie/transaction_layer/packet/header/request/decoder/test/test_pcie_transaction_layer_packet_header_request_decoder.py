...
class PcieTransactionLayerPacketHeaderRequestDecoderEnv(pyuvm.uvm_env):
    def build_phase(self):
        ...
        downstream_interface = pvm.interfaces.data_interface.DataInterface(cocotb.top.clk, (cocotb.top.request_requester_id, cocotb.top.request_tag, cocotb.top.request_error))
        self.downstream_monitor = pvm.monitors.monitor.Monitor('downstream_monitor', self, downstream_interface)
        self.model = pvm.models.ProtocolDecoderModel('model', self, pvm.decoders.PcieTransactionLayerPacketHeaderRequestDecoder(int(cocotb.top.UPSTREAM_WORD_WIDTH.value)))
...
@pyuvm.test()
class PcieTransactionLayerPacketHeaderRequestDecoderTest(pyuvm.uvm_test):
    def build_phase(self):
        max_word_index = 2 ** int(cocotb.top.UPSTREAM_WORD_INDEX_WIDTH.value)
        max_word = 2 ** int(cocotb.top.UPSTREAM_WORD_WIDTH.value) - 1
        self.env = PcieTransactionLayerPacketHeaderRequestDecoderEnv('env', self)
...
def test_pcie_transaction_layer_packet_header_request_decoder():
    runner = cocotb_tools.runner.get_runner('ghdl')

    current_dir = pathlib.Path().absolute()

    runner.test(test_module = 'test_pcie_transaction_layer_packet_header_request_decoder',
                hdl_toplevel = 'pcie_transaction_layer_packet_header_request_decoder_tb',
                hdl_toplevel_library = 'hdl_pcie_transaction_layer_packet_header_request_decoder_tb',
