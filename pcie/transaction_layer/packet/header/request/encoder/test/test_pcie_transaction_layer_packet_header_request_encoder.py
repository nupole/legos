...
class PcieTransactionLayerPacketHeaderRequestEncoderSequence(pvm.sequences.TransactionSequence):
    ...
    def _get_next_sequence_item(self, is_word):
        ...
        return pvm.sequences.data_sequence.DataSequenceItem('pcie_transaction_layer_packet_header_request_encoder_sequence', (word_index, requester_id, tag))

class PcieTransactionLayerPacketHeaderRequestEncoderEnv(pyuvm.uvm_env):
    def build_phase(self):
        self.upstream_sequencer = pyuvm.uvm_sequencer('upstream_sequencer', self)
        upstream_interface = pvm.interfaces.data_interface.DataInterface(cocotb.top.clk, (cocotb.top.downstream_word_index, cocotb.top.request_requester_id, cocotb.top.request_tag))
...
@pyuvm.test()
class PcieTransactionLayerPacketHeaderRequestEncoderTest(pyuvm.uvm_test):
    def build_phase(self):
        max_word_index = 2 ** int(cocotb.top.DOWNSTREAM_WORD_INDEX_WIDTH.value)
        self.env = PcieTransactionLayerPacketHeaderRequestEncoderEnv('env', self)
        self.upstream_sequence = PcieTransactionLayerPacketHeaderRequestEncoderSequence('upstream_sequence', is_word_list = [False, False, False, True], max_word_index = max_word_index)
...
def test_pcie_transaction_layer_packet_header_request_encoder():
    runner = cocotb_tools.runner.get_runner('ghdl')

    current_dir = pathlib.Path().absolute()

    runner.test(test_module = 'test_pcie_transaction_layer_packet_header_request_encoder',
                hdl_toplevel = 'pcie_transaction_layer_packet_header_request_encoder_tb',
                hdl_toplevel_library = 'hdl_pcie_transaction_layer_packet_header_request_encoder_tb',
