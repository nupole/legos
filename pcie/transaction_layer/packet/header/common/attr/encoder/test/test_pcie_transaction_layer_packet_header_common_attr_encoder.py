...
class PcieTransactionLayerPacketHeaderCommonAttrEncoderSequence(pvm.sequences.TransactionSequence):
    ...
    def _get_next_sequence_item(self, is_word):
        ...
        id_based_ordering = random.randint(0, 1)
        relaxed_ordering = random.randint(0, 1)
...
class PcieTransactionLayerPacketHeaderCommonAttrEncoderEnv(pyuvm.uvm_env):
    def build_phase(self):
        ...
        self.model = pvm.models.PcieTransactionLayerPacketHeaderCommonAttrEncoderModel('model', self)
...
def test_pcie_transaction_layer_packet_header_common_attr_encoder():
    runner = cocotb_tools.runner.get_runner('ghdl')

    current_dir = pathlib.Path().absolute()

    runner.test(test_module = 'test_pcie_transaction_layer_packet_header_common_attr_encoder',
                hdl_toplevel = 'pcie_transaction_layer_packet_header_common_attr_encoder_tb',
                hdl_toplevel_library = 'hdl_pcie_transaction_layer_packet_header_common_attr_encoder_tb',
