...
class PcieTransactionLayerPacketHeaderCommonDecoderEnv(pyuvm.uvm_env):
...
@pyuvm.test()
class PcieTransactionLayerPacketHeaderCommonDecoderTest(pyuvm.uvm_test):
    def build_phase(self):
        max_word_index = 2 ** int(cocotb.top.UPSTREAM_WORD_INDEX_WIDTH.value)
        max_word = 2 ** int(cocotb.top.UPSTREAM_WORD_WIDTH.value) - 1
        self.env = PcieTransactionLayerPacketHeaderCommonDecoderEnv('env', self)
...
def test_pcie_transaction_layer_packet_header_common_decoder():
    runner = cocotb_tools.runner.get_runner('ghdl')

    current_dir = pathlib.Path().absolute()

    runner.test(test_module = 'test_pcie_transaction_layer_packet_header_common_decoder',
                hdl_toplevel = 'pcie_transaction_layer_packet_header_common_decoder_tb',
                hdl_toplevel_library = 'hdl_pcie_transaction_layer_packet_header_common_decoder_tb',
