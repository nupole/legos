...
class PcieTransactionLayerPacketHeaderMemoryEncoderSequence(pvm.sequences.TransactionSequence):
    ...
    def _get_next_sequence_item(self, is_word):
        ...
        address = random.randint(0, 1073741823)
        ph = random.randint(0, 3)
        return pvm.sequences.data_sequence.DataSequenceItem('pcie_transaction_layer_packet_header_memory_encoder_sequence', (word_index, address, ph))

class PcieTransactionLayerPacketHeaderMemoryEncoderEnv(pyuvm.uvm_env):
    def build_phase(self):
        self.upstream_sequencer = pyuvm.uvm_sequencer('upstream_sequencer', self)
        upstream_interface = pvm.interfaces.data_interface.DataInterface(cocotb.top.clk, (cocotb.top.downstream_word_index, cocotb.top.memory_address, cocotb.top.memory_ph))
        ...
        self.model = pvm.models.ProtocolEncoderModel('model', self, pvm.encoders.PcieTransactionLayerPacketHeaderMemoryEncoder(int(cocotb.top.DOWNSTREAM_WORD_WIDTH.value)))
...
@pyuvm.test()
class PcieTransactionLayerPacketHeaderMemoryEncoderTest(pyuvm.uvm_test):
    def build_phase(self):
        max_word_index = 2 ** int(cocotb.top.DOWNSTREAM_WORD_INDEX_WIDTH.value)
        self.env = PcieTransactionLayerPacketHeaderMemoryEncoderEnv('env', self)
        self.upstream_sequence = PcieTransactionLayerPacketHeaderMemoryEncoderSequence('upstream_sequence', is_word_list = [False, False, False, True], max_word_index = max_word_index)
...
def test_pcie_transaction_layer_packet_header_memory_encoder():
    runner = cocotb_tools.runner.get_runner('ghdl')

    current_dir = pathlib.Path().absolute()

    runner.test(test_module = 'test_pcie_transaction_layer_packet_header_memory_encoder',
                hdl_toplevel = 'pcie_transaction_layer_packet_header_memory_encoder_tb',
                hdl_toplevel_library = 'hdl_pcie_transaction_layer_packet_header_memory_encoder_tb',
