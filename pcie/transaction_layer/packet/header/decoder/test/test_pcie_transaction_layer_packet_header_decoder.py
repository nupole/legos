...
class PcieTransactionLayerPacketHeaderDecoderEnv(pyuvm.uvm_env):
    def build_phase(self):
        ...
        downstream_interface = pvm.interfaces.data_interface.DataInterface(cocotb.top.clk, (cocotb.top.header_common_fmt_tlp_prefix, cocotb.top.header_common_fmt_data_indicator, cocotb.top.header_common_fmt_header_length, cocotb.top.header_common_packet_type, cocotb.top.header_common_tc, cocotb.top.header_common_attr_id_based_ordering, cocotb.top.header_common_attr_relaxed_ordering, cocotb.top.header_common_attr_no_snoop, cocotb.top.header_common_th, cocotb.top.header_common_td, cocotb.top.header_common_ep, cocotb.top.header_common_at, cocotb.top.header_common_length, cocotb.top.header_request_requester_id, cocotb.top.header_request_tag, cocotb.top.header_error))
        self.downstream_monitor = pvm.monitors.monitor.Monitor('downstream_monitor', self, downstream_interface)
        self.model = pvm.models.ProtocolDecoderModel('model', self, pvm.decoders.PcieTransactionLayerPacketHeaderDecoder(int(cocotb.top.UPSTREAM_WORD_WIDTH.value)))
...
@pyuvm.test()
class PcieTransactionLayerPacketHeaderDecoderTest(pyuvm.uvm_test):
    def build_phase(self):
        max_word_index = 2 ** int(cocotb.top.UPSTREAM_WORD_INDEX_WIDTH.value)
        max_word = 2 ** int(cocotb.top.UPSTREAM_WORD_WIDTH.value) - 1
        self.env = PcieTransactionLayerPacketHeaderDecoderEnv('env', self)
...
def test_pcie_transaction_layer_packet_header_decoder():
    runner = cocotb_tools.runner.get_runner('ghdl')

    current_dir = pathlib.Path().absolute()

    runner.test(test_module = 'test_pcie_transaction_layer_packet_header_decoder',
                hdl_toplevel = 'pcie_transaction_layer_packet_header_decoder_tb',
                hdl_toplevel_library = 'hdl_pcie_transaction_layer_packet_header_decoder_tb',
